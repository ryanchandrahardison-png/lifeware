...
- Option B rollout for `pages/actionItem.py`, `pages/delegationItem.py`, and `pages/calendarEvent.py` is implemented and frozen in the active stream.
...
- Project business-rule validation and mutation orchestration extraction into the dedicated state/service layer is COMPLETE in the active stream (with `pages/projectItem.py` using service-level save/delete orchestration entry points).

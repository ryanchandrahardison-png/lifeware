...
from core.project_state import (
    ...
    editor_text,
    get_delete_mode,
    get_draft_project_ui,
    load_project_editor,
    remove_draft_linked_item,
    set_delete_mode,
    sync_draft_runtime,
)
                ...
                if st.button(row_label, key=f"project_linked_compact::{project_id or 'draft'}::{group}::{idx}", use_container_width=True):
                    ...
                    if not selected_id and draft is not None:
                        _render_unresolved_warning(
                            ...
                            on_remove=lambda item=item: remove_draft_linked_item(draft=draft, item=item),
                        )
                    else:
        ...
        if selected_index is not None:
            ...
            if not selected_id and draft is not None:
                _render_unresolved_warning(
                    ...
                    warning="This linked item is still a draft and cannot be opened until the project is saved.",
                    remove_label="Remove Draft Linked Item",
                    on_remove=lambda item=selected_item: remove_draft_linked_item(draft=draft, item=item),
                )
            else:
...
if not is_edit:
    ...
    draft["due_date"] = editor_date_value(draft, "due_date")
    sync_draft_runtime(draft)
    ...
    editor = load_project_editor(project)
    ...
    if back:
        ...
        if not due_date_check.ok:
            ...
            result = update_project_from_editor(
                ...
                project_id=project_id,
                title=editor_text(editor, "title"),
                description=editor_text(editor, "description"),
                due_date=editor_date_value(editor, "due_date"),
                status=status,
            )
            if not result.ok:
                for error in result.errors or []:
